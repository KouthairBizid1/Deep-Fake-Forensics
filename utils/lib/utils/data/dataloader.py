...
class DataLoaderIter(object):
    ...
    def __getstate__(self):
        # Save the state excluding the data_queue and lock
        state = self.__dict__.copy()
        del state['data_queue']
        del state['lock']
        
        # If workers are used, indicate that the DataLoaderIterator is in a shutdown state
        if self.num_workers > 0:
            state['shutdown'] = True
    
            # Optionally, wait for workers to finish processing
            for w in self.workers:
                w.join()
